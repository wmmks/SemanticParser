...
ppos_model.add(Embedding(int(max_pos_features),128,mask_zero=True))

"""
dep_model = Sequential()
dep_model.add(Embedding(int(max_dependency_features),128,mask_zero=True))
"""

merged=Merge([word_model,pos_model,ppos_model],mode='concat')
...
final_model.fit([x_train,p_train,pp_train],y_train,batch_size=batch_size,validation_split=0.1,epochs=500)
		  

final_model.save_weights('model/WD2.mod')

final_model.load_weights('model/WD2.mod')

score, acc = final_model.evaluate([x_test,p_test,pp_test], y_test,batch_size=batch_size)
