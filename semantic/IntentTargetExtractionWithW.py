...
model=Sequential()
model.add(Embedding(int(max_features),128,mask_zero=True))
...
#model.fit(x_train,y_train,batch_size=batch_size,validation_split=0.1,epochs=30)

#model.save('model/test.mod')

model=load_model('model/test.mod')
score, ac = model.evaluate(x_test, y_test,batch_size=batch_size)
...
word_model.add(Embedding(int(max_features), 256))
word_model.add(Masking(mask_value=0, input_shape=(15, max_features)))
word_model.add(Dropout(0.5))
word_model.add(LSTM(512,return_sequences=True))
word_model.add(TimeDistributed(Dense(27, activation='softmax')))
pos_model = Sequential()
pos_model.add(Embedding(int(max_pos_features),256))
pos_model.add(Masking(mask_value=0, input_shape=(15, max_features)))
word_model.add(Dropout(0.5))
pos_model.add(LSTM(512,return_sequences=True))
pos_model.add(TimeDistributed(Dense(27, activation='softmax')))
...
final_model.fit([x_train,p_train],y_train,batch_size=batch_size,validation_split=0.1,epochs=30)
		  

final_model.save_weights('model/test3.mod')

#final_model.load_weights('model/test2.mod')
...
for i,data in enumerate(x_test):
	res = final_model.predict([data,p_test[i]])
	st=''
	for single_res in res:
		
		st+= str(argmax(single_res))+','
